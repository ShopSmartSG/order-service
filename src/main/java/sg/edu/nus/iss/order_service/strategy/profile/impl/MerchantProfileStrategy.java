...
@Component("merchant")
public class MerchantProfileStrategy extends Constants implements ProfileTypeStrategy {

    private static final Logger logger = LoggerFactory.getLogger(MerchantProfileStrategy.class);
    ...
    public Response getOrders(String listType, String id) {
        logger.info("Fetching orders for merchant with ID: {} and listType: {}", id, listType);

        JsonNode orders = orderTypeStrategyContext.getOrders(listType, "merchant", id).getData();

        if (orders == null || orders.isEmpty()) {
            logger.warn("No orders found for merchant with ID: {} and listType: {}", id, listType);
            return utils.getFailedResponse("No orders found for customer with ID: " + id + " and listType: " + listType);
        } else {
            logger.info("Successfully fetched {} orders for merchant with ID: {}", orders.size(), id);
